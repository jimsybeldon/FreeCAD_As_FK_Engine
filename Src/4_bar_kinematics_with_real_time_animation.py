...
# 1. Environment & Path Isolation (MUST HAPPEN BEFORE NUMPY/MATPLOTLIB IMPORTS)
freecad_bin_path = r"C:\Program Files\FreeCAD 1.1\bin"

# Prepend FreeCAD bin to PATH for DLL resolving
...
# Strip FreeCAD's bundled site-packages from sys.path to force .venv NumPy usage
sys.path = [p for p in sys.path if r"FreeCAD 1.1\bin\Lib\site-packages" not in p]

# Append FreeCAD bin so FreeCAD.pyd / Part.pyd are discoverable
if freecad_bin_path not in sys.path:
    sys.path.append(freecad_bin_path)

# 2. Safe Standard / Third-Party Imports
import math
import csv
import numpy as np
...
class FourBarAnalyticalEngine:
    ...
    def solve_kinematics(self, theta1_deg):
        ...
        if S > (self.L2 + self.L3) or S < abs(self.L2 - self.L3):
            return None  # Geometry unresolvable
...
def animate_mechanism(mechanism, trajectory_data, extrema):
    fig, ax = plt.subplots(figsize=(11, 9))

    path_x = [row["P"][0] for row in trajectory_data]
    path_y = [row["P"][1] for row in trajectory_data]

    # Static elements: Full coupler path
    ax.plot(path_x, path_y, color='#2b5c8f', label='Coupler Path (P)', linewidth=1.5, linestyle=':')

    # Ground line
    O_A = mechanism.O_A
    O_B = mechanism.O_B
    ax.plot([O_A[0], O_B[0]], [O_A[1], O_B[1]], 'k--', linewidth=2, label='Ground (O_A - O_B)')
    ax.scatter([O_A[0], O_B[0]], [O_A[1], O_B[1]], color='black', s=120, zorder=5)

    # Static Extrema Markers
    max_v, min_v = extrema["max_v"], extrema["min_v"]
    max_a, min_a = extrema["max_a"], extrema["min_a"]

    ax.scatter(max_v["P"][0], max_v["P"][1], color='red', marker='^', s=140, zorder=6,
               label=f'Max V ({max_v["vP_mag"]:.1f} mm/s)')
    ax.scatter(min_v["P"][0], min_v["P"][1], color='blue', marker='v', s=140, zorder=6,
               label=f'Min V ({min_v["vP_mag"]:.1f} mm/s)')
    ax.scatter(max_a["P"][0], max_a["P"][1], color='darkorange', marker='s', s=120, zorder=6,
               label=f'Max Accel ({max_a["aP_mag"]:.1f} mm/s²)')
    ax.scatter(min_a["P"][0], min_a["P"][1], color='purple', marker='d', s=120, zorder=6,
               label=f'Min Accel ({min_a["aP_mag"]:.1f} mm/s²)')

    for idx, z_at in enumerate(extrema["zero_at_list"]):
        ax.scatter(z_at["P"][0], z_at["P"][1], color='green', marker='o', s=80, zorder=7,
                   label='a_t = 0 (Tangential Accel Zero)' if idx == 0 else "")

    # Dynamic elements
    line_crank, = ax.plot([], [], color='#10b981', linewidth=3, label='Crank L1')
    line_coupler, = ax.plot([], [], color='#6366f1', linewidth=2, label='Coupler ABP')
    line_rocker, = ax.plot([], [], color='#ec4899', linewidth=3, label='Rocker L3')
    trace_path, = ax.plot([], [], color='red', linewidth=2, label='Traced Path')
    head_p = ax.scatter([], [], color='red', s=60, zorder=8)
    ...
    def update(frame_idx):
        nonlocal traced_x, traced_y
        state = trajectory_data[frame_idx]
        ...
        line_crank.set_data([O_A[0], A[0]], [O_A[1], A[1]])
        line_coupler.set_data([A[0], B[0], P[0], A[0]], [A[1], B[1], P[1], A[1]])
        line_rocker.set_data([B[0], O_B[0]], [B[1], O_B[1]])
        ...
        if len(traced_x) > 360:
            traced_x.pop(0)
            traced_y.pop(0)
        trace_path.set_data(traced_x, traced_y)

        # Update current point P
        head_p.set_offsets([P[0], P[1]])

        title_text.set_text(
            f'4-Bar Real-Time Kinematics | Crank θ1 = {state["theta1_deg"]:.1f}° | '
            f'|V| = {state["vP_mag"]:.1f} mm/s | |A| = {state["aP_mag"]:.1f} mm/s²'
        )

        return line_crank, line_coupler, line_rocker, trace_path, head_p, title_text
...
if __name__ == "__main__":
    mechanism = FourBarAnalyticalEngine(
        L1=50.0, L2=120.0, L3=100.0, d=140.0,
        LP=80.0, alpha_deg=30.0, omega1=10.0, O_A=(0.0, 0.0)
    )

    # 1. Generate 360 frames (1-degree increments)
    trajectory = []
    for deg in range(360):
        sol = mechanism.solve_kinematics(float(deg))
        if sol:
    ...
    # 3. Launch Real-Time Animation
    animate_mechanism(mechanism, trajectory, extrema)